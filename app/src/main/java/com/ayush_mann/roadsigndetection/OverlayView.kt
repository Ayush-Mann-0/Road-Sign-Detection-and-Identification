...
class OverlayView @JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    private fun formatLabel(name: String): String {
        ...
        return if (lower.contains("speedlimit") || lower.contains("speed_limit")) {
            "speed limit"
        } else {
            name.replace("_", " ")
        }
    ...
    private fun drawBoundingBox(
        ...
        val formattedName = formatLabel(boundingBox.clsName)
        val mainLabel = "$formattedName $confidence%"

        // Scale text size according to length
        val baseTextSize = 36f
        val scaledTextSize = if (mainLabel.length > 15) {
            (baseTextSize * (15f / mainLabel.length)).coerceAtLeast(22f)
        } else {
            baseTextSize
        }
